use crate::vcs::{self, Diff, DiffFileInner, DiffFile, DiffHunk, DiffLineInner};
use std::str;
use std::path::PathBuf;
use git2::{DiffFindOptions, DiffFormat, DiffOptions, Repository, Oid};
use anyhow::Result;


pub fn diff_from_to(repo: &Repository, from: Option<&vcs::Commit>, to: Option<&vcs::Commit>) -> Result<Diff> {
    let mut old = None;
    let mut new = None;
